...
let pallete = ["#8c1dff", "#f223ff", "#ff2976", "#ff901f", "#ffd318"];

export default function getMaterial({ asciiTexture, customLength, scene }) {
  ...
  const asciiCode = Fn(() => {
    ...
    const asciiUV = vec2(
      mix(glyphInset, float(1).sub(glyphInset), uv().x)
        .div(customLength)
        .add(floor(brightness.mul(customLength)).div(customLength)),
      mix(glyphInset, float(1).sub(glyphInset), uv().y),
    );
