...
import { detectWorkspace, mergeJsonConfig, mergeTsdownConfig, rewriteEslint, rewriteImportsInDirectory, rewritePrettier, rewriteScripts } from "../../binding/index.js";
...
function rewriteLintStagedConfigFile(projectPath, report) {
	...
	if (hasUnsupported) infoMigration("Only \"staged\" in vite.config.ts is supported. See https://viteplus.dev/migration/#lint-staged", report);
}
/**
...
function mergeTsdownConfigFile(projectPath, silent = false, report) {
	...
	infoMigration(`Please manually merge ${displayRelative(fullTsdownConfigPath)} into ${displayRelative(fullViteConfigPath)}, see https://viteplus.dev/migration/#tsdown`, report);
}
/**
...
function mergeStagedConfigToViteConfig(projectPath, stagedConfig, silent = false, report) {
	...
	if (result.updated) {
		...
		infoMigration(`Please add staged config to ${displayRelative(fullViteConfigPath)} manually, see https://viteplus.dev/config/`, report);
		return false;
	}
}
/**
...
function setupGitHooks(projectPath, oldHooksDir, silent = false, report) {
	...
	if (!stagedMerged && !hasStandaloneConfig) {
		const stagedConfig = readJsonFile(packageJsonPath)?.["lint-staged"] ?? { "*": "vp check --fix" };
	...
	if (oldHooksDir) {
		...
		if (existingPath === `${oldHooksDir}/_` || existingPath === oldHooksDir) spawn.sync("git", [
			"config",
			"--local",
			"--unset",
			"core.hooksPath"
...
function mergeAndWriteEditorConfig(filePath, incoming, fileName, displayPath, silent = false) {
	writeJsonFile(filePath, mergeEditorConfigs(readJsonFile(filePath), incoming, fileName));
...
function mergeEditorConfigs(existing, incoming, fileName) {
	...
	for (const [key, value] of Object.entries(incoming)) if (!(key in result)) result[key] = value;
	else if (typeof result[key] === "object" && result[key] !== null && !Array.isArray(result[key]) && typeof value === "object" && value !== null && !Array.isArray(value)) result[key] = {
		...value,
		...result[key]
	};
	return result;
...
//#endregion
export { rewriteMonorepo as _, detectExistingEditor as a, createMigrationReport as b, checkViteVersion as c, detectPrettierProject as d, installGitHooks as f, preflightGitHooksSetup as g, migratePrettierToOxfmt as h, detectEditorConflicts as i, checkVitestVersion as l, migrateEslintToOxlint as m, updatePackageJsonWithDeps as n, selectEditor as o, mergeViteConfigFiles as p, updateWorkspaceConfig as r, writeEditorConfigs as s, detectWorkspace$1 as t, detectEslintProject as u, rewriteMonorepoProject as v, require_semver as x, rewriteStandaloneProject as y };
