...
function checkPackageVersion(projectPath, name, minVersion) {
    ...
    if (semver.satisfies(metadata.version, `<${minVersion}`)) {
        ...
        prompts.log.error(`✘ ${name}@${metadata.version} in ${displayRelative(packageJsonFilePath)} is not supported by auto migration`);
...
export async function migrateEslintToOxlint(projectPath, interactive, eslintConfigFile, packages, options) {
    ...
    if (eslintConfigFile) {
        ...
        const replaceOk = await runOxlintMigrateStep(vpBin, projectPath, migratePackage, ['--replace-eslint-comments'], spinner, 'ESLint comment replacement failed', `You can run \`vp dlx ${migratePackage} --replace-eslint-comments\` manually later`);
        if (replaceOk) {
...
export async function migratePrettierToOxfmt(projectPath, interactive, prettierConfigFile, packages, options) {
    ...
    if (prettierConfigFile) {
        ...
        try {
            ...
            const migrateOk = await runPrettierMigrateStep(vpBin, projectPath, spinner, 'Prettier migration failed', 'You can run `vp fmt --migrate=prettier` manually later');
            if (!migrateOk) {
                return false;
            }
 ...
 * @param projectPath - The path to the project
 */
export function rewriteStandaloneProject(projectPath, workspaceInfo, skipStagedMigration, silent = false, report) {
    const packageJsonPath = path.join(projectPath, 'package.json');
    if (!fs.existsSync(packageJsonPath)) {
        return;
    }
...
export function rewriteMonorepo(workspaceInfo, skipStagedMigration, silent = false, report) {
    ...
    for (const pkg of workspaceInfo.packages) {
        rewriteMonorepoProject(path.join(workspaceInfo.rootDir, pkg.path), workspaceInfo.packageManager, skipStagedMigration, silent, report);
    }
    if (!skipStagedMigration) {
...
function rewriteRootWorkspacePackageJson(projectPath, packageManager, skipStagedMigration) {
    ...
    editJsonFile(packageJsonPath, (pkg) => {
        ...
        if (!pkg.devDependencies?.[VITE_PLUS_NAME]) {
            pkg.devDependencies = {
                ...pkg.devDependencies,
                [VITE_PLUS_NAME]: packageManager === PackageManager.npm || VITE_PLUS_VERSION.startsWith('file:')
                    ? VITE_PLUS_VERSION
                    : 'catalog:',
            };
        }
...
function rewriteLintStagedConfigFile(projectPath, report) {
    ...
    for (const filename of LINT_STAGED_JSON_CONFIG_FILES) {
        ...
        if (!hasStagedConfigInViteConfig(projectPath)) {
            ...
            warnMigration(`${displayRelative(configPath)} found but "staged" already exists in vite.config.ts — please merge manually`, report);
        }
    }
    ...
    if (hasUnsupported) {
        infoMigration('Only "staged" in vite.config.ts is supported. See https://viteplus.dev/guide/migrate#lint-staged', report);
    }
}
/**
...
function mergeTsdownConfigFile(projectPath, silent = false, report) {
    ...
    if (result.updated) {
        ...
        if (!silent) {
            prompts.log.success(`✔ Added import for ${displayRelative(fullTsdownConfigPath)} in ${displayRelative(fullViteConfigPath)}`);
        }
    }
...
function mergeAndRemoveJsonConfig(projectPath, viteConfigPath, jsonConfigPath, configKey, silent = false, report) {
    ...
    if (result.updated) {
        ...
        if (!silent) {
            prompts.log.success(`✔ Merged ${displayRelative(fullJsonConfigPath)} into ${displayRelative(fullViteConfigPath)}`);
        }
    }
    else {
...
function rewriteAllImports(projectPath, silent = false, report) {
    ...
    if (errors > 0) {
        if (report) {
            warnMigration(`${errors === 1 ? 'one file had an error' : `${errors} files had errors`} while rewriting imports`, report);
        }
        else {
