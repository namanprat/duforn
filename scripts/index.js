...
function initIndex() {
  // Initialize Spotlight Galleries
  const galleries = document.querySelectorAll(".spotlight-gallery");
  ...
  // Spotlight Background Fade Effect
  const spotlightSection = document.querySelector('.spotlight');
  if (spotlightSection) {
    // Ensure section is positioned for absolute child and has transparent background
    gsap.set(spotlightSection, { 
        position: 'relative', 
    ...
    overlay.style.backgroundColor = 'var(--theme-background)';
    overlay.style.zIndex = '-1';
    overlay.style.opacity = '0'; // Start transparent
    overlay.style.pointerEvents = 'none';
    overlay.style.maskImage = 'linear-gradient(to bottom, transparent 0%, black 25%, black 100%)';
    overlay.style.webkitMaskImage = 'linear-gradient(to bottom, transparent 0%, black 25%, black 100%)';
    
    // Prepend to ensure it's early in DOM, though z-index handles layering
    spotlightSection.prepend(overlay);

    // Fade in opacity pure alpha change
    gsap.to(overlay, {
      opacity: 1,
      scrollTrigger: {
