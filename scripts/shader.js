...
export const fragmentShader = `
  ...
  void main() {
    ...
    float borderIntensity = (1.0 - gridMask);
    
    if (borderIntensity > 0.001) {
        color = borderRGB;
        alpha = borderAlpha * borderIntensity;
    }
    ...
    if (inImageArea && imageAlpha > 0.0) {
      ...
      alpha = mix(alpha, 1.0, imageAlpha);
    }
    ...
    if (inTextArea) {
      ...
      alpha = mix(alpha, 1.0, textColor.a);
    }
    
    float fade = 1.0 - smoothstep(1.2, 1.8, radius);
    
    gl_FragColor = vec4(color * fade, alpha * fade);
  }
`;
