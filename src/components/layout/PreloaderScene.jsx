...
import {
  ...
  GrainShader,
  VignetteShader,
  createPostFXUniforms,
} from "../../../scripts/shaders/post-fx.js";
...
function buildLetterLayout(text, layoutSeed) {
  ...
  const totalWidth = chars.reduce(
    (sum, char) => sum + (char === " " ? SPACE_WIDTH : LETTER_WIDTH),
    0,
  );
      ...
      if (phase === "out") {
        ...
        const flicker =
          localElapsed <= duration
            ? Math.sin(localElapsed * letter.flickerRate) > letter.flickerCutoff
              ? 0.3
              : 1
            : 0;
